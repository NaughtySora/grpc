...
module.exports = (options = {}) => {
  ...
  const onBind = error => {
    if (error) logger.error(`${SERVER_INFO} binding failed`, error);
  ...
  return {
    async start(services = []) {
      ...
      const tunnel = Array.isArray(ssl) ? createSsl(...ssl) : createInsecure();
    ...
    stop(ms = 5000) {
      ...
      server.tryShutdown(error => {
        if (error) return void logger.error(`${SERVER_INFO} has errored`, error);
